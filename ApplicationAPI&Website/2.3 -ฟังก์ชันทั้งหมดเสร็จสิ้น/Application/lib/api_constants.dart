...
class APIConstants {
  static const String baseURL = String.fromEnvironment(
    'API_BASE_URL',
    defaultValue: 'http://10.50.8.246:4000',
  );
  ...
  static String getPBox1AttendanceSummaryEndpoint(String courseCode,
          String section, String startDate, String endDate) =>
      '/api/pbox1-attendance-summary/$courseCode/$section/$startDate/$endDate';

  static Future<Map<String, dynamic>> getPBox1AttendanceSummary(
      String courseCode,
      String section,
      String startDate,
      String endDate) async {
    try {
      final response = await makeRequest(getPBox1AttendanceSummaryEndpoint(
          courseCode, section, startDate, endDate));
      if (response.statusCode == 200) {
        return jsonDecode(response.body);
      } else {
        throw Exception(
            'Failed to load PBox1 attendance summary: ${response.statusCode}');
      }
    } catch (e) {
      print('Error in getPBox1AttendanceSummary: $e');
      rethrow;
    }
  }

  static String getPBox1TeacherCoursesEndpoint(String teacherId) =>
      '/api/pbox1-teacher-courses/$teacherId';

  static Future<List<Map<String, dynamic>>> getPBox1TeacherCourses(
      String teacherId) async {
    try {
      final response =
          await makeRequest(getPBox1TeacherCoursesEndpoint(teacherId));
      if (response.statusCode == 200) {
        final List<dynamic> coursesJson = jsonDecode(response.body);
        return coursesJson
        ...
        throw Exception(
            'Failed to load PBox1 teacher courses: ${response.statusCode}');
      }
    } catch (e) {
      print('Error in getPBox1TeacherCourses: $e');
      rethrow;
    }
  }
}
