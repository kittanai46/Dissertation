...
class Sbox3 extends StatefulWidget {
  ...
  const Sbox3({Key? key, required this.idNumber}) : super(key: key);
...
class _Sbox3State extends State<Sbox3> {
  ...
  Future<void> fetchCourseNames() async {
    try {
      final courseCodes = attendanceData
          .map((record) => record['course_code'].toString())
          .toSet()
          .toList();
      courseNames = await APIConstants.getCoursesNames(courseCodes);
      setState(() {
        isLoading = false;
      });
    } catch (e) {
