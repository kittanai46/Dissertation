...
class _HomePageState extends State<HomePage> {
  ...
  final String targetUUID =
      "952f7b6f622c04a2a8407abf6c719dba"; // เปลี่ยนเป็น UUID ที่ต้องการค้นหา
      ...
      FlutterBluePlus.scanResults.listen((results) {
        for (ScanResult result in results) {
          if (result.advertisementData.manufacturerData.isNotEmpty) {
            String? scannedUUID =
                getUuid(result.advertisementData.manufacturerData.values.first);
            print('Scanned UUID: $scannedUUID');
  ...
  void showTargetDeviceFoundDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('พบอุปกรณ์ที่มี UUID $targetUUID'),
        actions: [
          TextButton(
            child: const Text('ตกลง'),
